...
export const Button: FC<ButtonHTMLAttributes<any> & ButtonProps & Space & Layout & Decor> = ({
  onClick,
  className = "",
  href,
  target,
  ...
  const classNames = `button ${icon ? "icon" : ""} ${secondary ? "secondary" : ""} ${small ? "small" : ""} ${
      large ? "large" : ""
  } ${className}`.trim();
        ...
        {href
         ? <Link href={href}>
           <a tabIndex={0}
              ...
              onClick={onClick}
              onKeyDown={onClick}
              {...nonCssProps}>{children}</a>
         </Link>
         : <button type="button" className={classNames} onClick={onClick} {...nonCssProps}>{children}</button>
        }
        <style jsx>{`
          .button {
            position: relative;
