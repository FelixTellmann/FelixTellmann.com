...
export const Button: FC<ButtonHTMLAttributes<any> & ButtonProps & Space & Layout & Decor> = ({ onClick, className = '', href, target, icon, secondary, small, large, children, ...props }) => {
  ...
  return <>
    {
      href
      ? <Link href={href}><a title={label} aria-label={href.includes('mailto') ? href.replace('mailto', '') : label} target={target} rel={target === '_blank' ? 'noopener noreferrer' : ''} className={classNames} onClick={onClick} {...nonCssProps}>{children}</a></Link>
