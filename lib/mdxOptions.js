      ...
      mdxContent.match(/#+\s+.*/gi).reduce((accumulator, heading) => {
        ...
        if (acc && acc[acc.length - 1] && acc[acc.length - 1].level < heading.split(' ')[0].length) {
          if (
            ...
            acc[acc.length - 1].subheading[acc[acc.length - 1].subheading.length - 1].level <
              heading.split(' ')[0].length
          ) {
...
const mdxOptions = {
  ...
  remarkPlugins: [
    ...
    [
      ...
      { behavior: 'wrap', linkProperties: { ariaHidden: true, tabIndex: -1, className: ['autolink-heading'] } }
    ],
