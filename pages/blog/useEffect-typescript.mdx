---
title: 'Managing side effects in React with the useEffect hook'
...
published: true
excerpt: 'Check out how the useEffect hook works and how to supercharge it with Typescript.'
...
to track a global event. The `return () => { document.removeEventListener("mousemove", onMouseMove) }`
part runs on `componentWillUnmount` and allows you to clean up the eventListener once it is
not needed anymore.
